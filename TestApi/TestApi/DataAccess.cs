...
public class DataAccess
{
    ...
    public List<UserData> GetUserData(string command)
    {
        ...
        while (reader.Read())
        {
            data.Add(new UserData { Id = reader.GetInt32(0), Name = reader.GetString(1), Alias = reader.GetString(2), GameData = new() { Id = reader.GetInt32(3), HighScore = reader.GetInt32(4), LevelsCompleted = reader.GetInt32(5), TimePlayedInSeconds = reader.GetInt32(6) } });
        }
        reader.Close();
        conn.Close();
    ...
    public void PutGameData(UserData data)
    {
        ...
        SQLiteCommand cmd = new(conn);
        cmd.CommandText = $"UPDATE UserData SET Name = '{data.Name}', Alias = '{data.Alias}' WHERE Id = {data.Id}";
        cmd.ExecuteNonQuery();
